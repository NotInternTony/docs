...
When `CUBEPAY_API_KEY` is not set and `NODE_ENV` is not `production`, the SDK service returns **mock session data** instead of calling the API. This lets you develop and test your UI without real credentials.

Mock sessions use IDs prefixed with `mock-session-` and simulate the full payment flow locally.
