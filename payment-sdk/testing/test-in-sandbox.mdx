...
NEXT_PUBLIC_CUBEPAY_API_HOST=https://api.sandbox.pay.grain.inc
```
...
When `CUBEPAY_API_KEY` is not set and `NODE_ENV` is not `production`, the SDK automatically returns **mock session data**. This lets you build and test your UI without any API credentials.

Mock sessions:
- Use IDs prefixed with `mock-session-`
- Return simulated responses for `GET /api/payment-sessions`
- Skip real API calls entirely

```typescript
// In development without credentials, this returns mock data
...
<Tip>
  Development mode is useful for UI development and component testing. Use the full sandbox environment for integration testing before going live.
</Tip>
